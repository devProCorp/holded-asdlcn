  ...
  const columns = [
    ...
    { key: 'total', label: 'Total', sortable: true, render: (row) => <span className="font-medium">{formatCurrency(row.total)}</span> },
    {
      ...
      render: (row) => {
        ...
        return pending > 0 ? <span className="text-orange-600 font-medium">{formatCurrency(pending)}</span> : <span className="text-green-600">-</span>;
      },
    },
    {
      key: 'status',
      ...
      render: (row) => {
        ...
        if (row.draft) return <span className="inline-block px-2 py-0.5 rounded-full text-xs font-medium bg-gray-100 text-gray-600">Borrador</span>;
        if (isPaid) return <span className="inline-block px-2 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-700">Pagado</span>;
        return <span className="inline-block px-2 py-0.5 rounded-full text-xs font-medium bg-orange-100 text-orange-700">Pendiente</span>;
      },
    },
    {
      key: 'pdf',
        ...
        <>
          ...
          <div className="flex gap-6 flex-wrap text-sm">
            <div className="bg-white border border-gray-200 rounded-lg px-4 py-2">
              <span className="text-gray-500">Documentos:</span> <span className="font-medium">{docs.length}</span>
            </div>
            <div className="bg-white border border-gray-200 rounded-lg px-4 py-2">
              <span className="text-gray-500">Total:</span> <span className="font-medium">{formatCurrency(totalAmount)}</span>
            </div>
            <div className="bg-white border border-gray-200 rounded-lg px-4 py-2">
              <span className="text-gray-500">Cobrado/Pagado:</span> <span className="font-medium text-green-600">{formatCurrency(totalPaid)}</span>
            </div>
            <div className="bg-white border border-gray-200 rounded-lg px-4 py-2">
              <span className="text-gray-500">Pendiente:</span> <span className="font-medium text-orange-600">{formatCurrency(totalPending)}</span>
            </div>
          </div>
