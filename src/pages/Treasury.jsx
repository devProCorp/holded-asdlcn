  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-4">
        ...
        <div className="text-right space-y-1">
          ...
          <div className="flex gap-4 text-xs">
            <span className="text-green-600">Disponible: {formatCurrency(positiveBalance)}</span>
            <span className="text-red-600">Deuda: {formatCurrency(negativeBalance)}</span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <div key={account.id} className="bg-white rounded-xl border border-gray-200 p-5">
              <div className="flex items-center gap-3 mb-3">
                <div className={`p-2 rounded-lg ${balance >= 0 ? 'bg-blue-50' : 'bg-red-50'}`}>
                  <Icon size={20} className={balance >= 0 ? 'text-blue-600' : 'text-red-500'} />
                </div>
                <div className="min-w-0 flex-1">
                  <p className="font-medium text-sm truncate" title={account.name}>{account.name}</p>
                  <p className="text-xs text-gray-400">{typeLabels[account.type] || account.type} - {account.treasuryName || ''}</p>
                </div>
              </div>
              ...
              <div className="mt-2 space-y-1">
                {account.iban && (
                  <p className="text-xs text-gray-400 font-mono">{account.iban}</p>
                )}
                {account.accountNumber && (
                  <p className="text-xs text-gray-400">Cuenta: {account.accountNumber}</p>
                )}
              </div>
            </div>
          );
        ...
        {accounts.length === 0 && (
          <p className="text-gray-400 col-span-full text-center py-10">No hay cuentas de tesoreria</p>
        )}
      </div>
    </div>
  );
